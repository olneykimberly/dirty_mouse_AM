...
info <- extract_gene_cpm("Lcn2", cpm_gene_names, info)
...
plot_data <- info %>%
  # Use backticks around Hbb-bs
  dplyr::select(sample, group, Uty, Lcn2, `Hbb-bs`, Ttr) %>%
  # Using pivot_longer (the modern replacement for gather)
  tidyr::pivot_longer(
    cols = c(Uty, Lcn2, `Hbb-bs`, Ttr), 
    names_to = "Gene", 
    values_to = "Expression"
  ) %>%
  dplyr::mutate(
    group = factor(group, levels = c("clean_WT", "clean_AM", "dirty_WT", "dirty_AM")),
    Gene = factor(Gene, levels = c("Uty", "Lcn2", "Hbb-bs", "Ttr"))
  )

# Create a list of plots, one for each gene
gene_plots <- list()
genes_to_plot <- c("Uty", "Lcn2", "Hbb-bs", "Ttr") # Only plotting the newly requested genes

condition_colors <- c(clean_WT = "#56B4E9", 
clean_AM = "#009E73", 
dirty_WT = "#F0E442", 
dirty_AM = "#E69F00")
...
```{r varpart}
form_varPart <- ~ (1|C_or_D) + (1|genotype) + (1|sex) + (1 | collection_date) + RIN 
...
```{r}
form <- ~ group + genotype + C_or_D + sex + collection_date + RIN
...
p <- info %>%
    ...
    scale_fill_manual(values = c("clean_WT" = "#56B4E9", "clean_AM" = "#009E73", "dirty_WT" = "#F0E442", "dirty_AM" = "#E69F00")) +
    theme_minimal() +
    theme(
      legend.position = "none",
      plot.title = element_text(hjust = 0.5, face = "bold"),
...
list_of_datasets <- list(
  "clean_AM_vs_clean_WT" = clean_AM_vs_clean_WT,
  "dirty_AM_vs_dirty_WT" = dirty_AM_vs_dirty_WT,
  "dirty_WT_vs_clean_WT" = dirty_WT_vs_clean_WT,
  "dirty_AM_vs_clean_AM" = dirty_AM_vs_clean_AM
)
...
```{r}
# sex as a covariate
clean_AM_vs_clean_WT_up <- subset(clean_AM_vs_clean_WT, logFC > 0.0 & adj.P.Val <= 0.05)
clean_AM_vs_clean_WT_down <- subset(clean_AM_vs_clean_WT, logFC < -0.0 & adj.P.Val <= 0.05)
dirty_AM_vs_dirty_WT_up <- subset(dirty_AM_vs_dirty_WT, logFC > 0.0 & adj.P.Val <= 0.05)
dirty_AM_vs_dirty_WT_down <- subset(dirty_AM_vs_dirty_WT, logFC < -0.0 & adj.P.Val <= 0.05)
```

# Upset of DEGs
```{r}
library(ComplexUpset)
list_input <- list("clean AM vs WT down-regulated" = clean_AM_vs_clean_WT_down$gene_name,
                   "clean AM vs WT up-regulated" = clean_AM_vs_clean_WT_up$gene_name,
                   "dirty AM vs WT down-regulated" = dirty_AM_vs_dirty_WT_down$gene_name,
                   "dirty AM vs WT up-regulated" = dirty_AM_vs_dirty_WT_up$gene_name)
data <- fromList(list_input)

upset_gene <- upset(data, set_sizes = FALSE,
      c('clean AM vs WT down-regulated',
        'dirty AM vs WT down-regulated',
        'clean AM vs WT up-regulated',
        'dirty AM vs WT up-regulated'),
      themes=upset_modify_themes(
        list('intersections_matrix'=theme(text=element_text(size=10),
          ...
          'overall_sizes' = theme(axis.text.x = element_text(size =10), 
              plot.margin = margin(0, 0, 0, 0, "cm")))),
  queries=list(
    upset_query(set='clean AM vs WT down-regulated', fill='blue'),
    upset_query(set='dirty AM vs WT down-regulated', fill='blue'),
    upset_query(set='clean AM vs WT up-regulated', fill='red'),
    upset_query(set='dirty AM vs WT up-regulated', fill='red')
  ),
  intersections = list(
                       c('clean AM vs WT down-regulated', 'dirty AM vs WT down-regulated'), 
                       'clean AM vs WT down-regulated',
                       'dirty AM vs WT down-regulated', 
                       c('clean AM vs WT up-regulated', 'dirty AM vs WT up-regulated'), 
                       'clean AM vs WT up-regulated',
                       'dirty AM vs WT up-regulated'
  ),
