...
with open('sampleReadGroupInfo.txt', 'r') as infile:
    for line in infile:
        ...
        line = line.replace("-", "_")
        split = line.split()
        sampleAttributes = split[0].split('_')  # AM_CF6AM1_1_BR_Whole_C1_WGMRS_A44774_235MMJLT4_AATGGTACCT_L002_R1_001.fastq.gz
...
header = '''{{
    ...
    "rawReads" : "/tgen_labs/jfryer/projects/dirty_mice/AM/bulkRNA",
    "rawQC" : "../rawQC/",
...
with open('sampleReadGroupInfo.txt', 'r') as infile:
    for line in infile:
        ...
        out = '''
    "{0}":{{
        "fq_path": "/tgen_labs/jfryer/projects/dirty_mice/AM/bulkRNA/",
        "fq1": "{1}",
        "fq2": "{2}",
        "ID": "{3}",
        "SM": "{4}",
